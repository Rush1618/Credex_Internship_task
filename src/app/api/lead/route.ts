  ...
  try {
    ...
    if (resendKey && fromEmail) {
      try {
        ...
        try {
          console.log(`[LeadAPI] Sending report to: ${email}`);
          ...
          if (userEmailResult.error) {
            console.error('[LeadAPI] User email error:', userEmailResult.error);
          } else {
            console.log('[LeadAPI] User email sent successfully:', userEmailResult.data?.id);
          }
        } catch (userErr) {
        ...
        if (isHighSavings && adminEmail) {
          try {
            ...
            if (adminEmailResult.error) {
              console.error('[LeadAPI] Admin email error:', adminEmailResult.error);
            } else {
              console.log('[LeadAPI] Admin email sent successfully:', adminEmailResult.data?.id);
            }
          } catch (adminErr) {
        ...
        console.error('[LeadAPI] Resend library error:', importErr);
      }
    }
